...
public class DatabaseHelper extends SQLiteOpenHelper {
    ...
    public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
        if (oldVersion < 2) {db.execSQL("DROP TABLE IF EXISTS " + TABLE_STORAGE);
            db.execSQL(CREATE_TABLE_RECIPES_DESCRIPTION);
            db.execSQL(CREATE_TABLE_RECIPES_INSTRUCTION);
        }
    }
    ...
    public long insertRecipeDescription(String name, String category, String allergy, double protein, double fat, double carbs, String microNutrients, String description) {
        SQLiteDatabase db = this.getWritableDatabase();
        ContentValues values = new ContentValues();
        values.put("name", name);
    ...
    public boolean insertRecipeInstruction(long id, String name, String ingredient1, String ingredient2, String ingredient3, String instructions) {
        SQLiteDatabase db = this.getWritableDatabase();
        ContentValues values = new ContentValues();
        ...
        if (cursor != null) {
            try {
                while (cursor.moveToNext()) {
                    ...
                    recipeList.add(new RecipeDescription(id, name, category, allergy, description));
    ...
    public RecipeDescription getRecipeById(int recipeId) {
        ...
        String query = "SELECT id, name, category, allergy, description, protein, fat, carbs, micro_nutrients FROM recipes_description WHERE id = ?";
        ...
        if (cursor.moveToFirst()) {
            ...
            String instructionQuery = "SELECT instructions FROM recipes_instruction WHERE id = ?";
